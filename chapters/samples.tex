...
%\begin{table}[htbp]
%  \centering
%  \input{tab/samples/dataSamples.tex}
%  \caption{
...
% Signal samples
%This analysis makes use of ten benchmark signal models, which are listed in tables~\ref{tab:ggFGBulkToWWSamples}-\ref{tab:VBFZprToWWSamples} with their cross sections and branching fractions where appropriate.
%The MC samples used to simulate events produced by the narrow resonance models are listed in table~\ref{tab:sigSamples} with their branching fractions where appropriate, while the cross sections for each signal model are displayed in figure~\ref{fig:sigCrossSec} as a function of the resonance mass \MX.
%The branching fractions only apply to signal samples for which other final states that are not semileptonic are present, whereas samples without a branching fraction listed are entirely restricted to the semileptonic state.
...
The \ggF/\VBF\GBulktoWW model assumes a curvature of $\tilde{k}=0.5$, and the cross sections for \ggF\GBulktoWW are next-to-leading-order (NLO), while those of the \VBF process are LO.
...
Meanwhile, the \VBF\ZprtoWW and \DY\WprtoWZ samples use cross sections from the HVT model C, with $g_V\approx1$, $c_H\approx1$, and $c_F=0$.%, which are taken from this reference\footnote{\url{https://github.com/zucchett/HVT/blob/master/dataframe.csv}}.
...
%\begin{table}[htbp]
%  \centering
%  \input{tab/samples/sigSamples.tex}
%  \caption{
    %Samples for each of the nine benchmark signals with branching fractions where appropriate.
    %The samples without branching fractions are already restricted to the semileptonic final state.
...
%  \caption{
%    Samples for the \ggF\GBulktoWW signal with cross sections and branching fractions.
...
%  \caption{
%    Samples for the \VBF\GBulktoWW signal with cross sections and branching fractions.
...
%  \caption{
%    Samples for the \ggF\RadtoWW signal with cross sections and branching fractions.
...
%  \caption{
%    Samples for the \VBF\RadtoWW signal with cross sections and branching fractions.
...
%  \caption{
%    Samples for the \DY\WprtoWZ signal with cross sections and branching fractions.
%    ``\texttt{[SUFFIX]}'' is \texttt{13TeV-madgraph} for the Summer16 campaign, and \texttt{TuneCP5\_13TeV-madgraph-pythia8} for Fall17 and Autumn18.
%  }
...
%  \caption{
%    Samples for the \VBF\WprtoWZ signal with cross sections and branching fractions.
%    ``\texttt{[SUFFIX]}'' is \texttt{13TeV-madgraph-pythia8} for the Summer16 campaign, \texttt{TuneCP5\_13TeV-madgraph} for Fall17, and \texttt{TuneCP5\_PSweights\_13TeV-madgraph} for Autumn18.
...
%  \caption{
%    Samples for the \DY\WprtoWH signal with cross sections and branching fractions.
...
%  \caption{
%    Samples for the \VBF\WprtoWH signal with cross sections and branching fractions.
%  }
...
%  \caption{
%    Samples for the \DY\ZprtoWW signal with cross sections and branching fractions.
%    ``\texttt{[SUFFIX]}'' is \texttt{13TeV-madgraph} for the Summer16 campaign, and \texttt{TuneCP5\_13TeV-madgraph} for Fall17 and Autumn18.
%  }
...
%  \caption{
%    Samples for the \VBF\ZprtoWW signal with cross sections and branching fractions.
%    ``\texttt{[SUFFIX]}'' is \texttt{13TeV-madgraph-pythia8} for the Summer16 campaign, \texttt{TuneCP5\_13TeV-madgraph} for Fall17, and \texttt{TuneCP5\_PSweights\_13TeV-madgraph} for Autumn18.
...
% Background samples
The MC samples used to simulate SM background contributions listed in table~\ref{tab:bkgSamples} along with their cross sections where available. %~\ref{tab:bkg2016Samples} and \ref{tab:bkg2017Samples} along with their cross sections.

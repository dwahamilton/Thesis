...
% Saturated model
The goodness-of-fit is estimated using the saturated model (Cousins-Baker) algorithm, in which 1000 background toys are generated for the g.o.f estimator.
The process is repeated for the data, which is compared to the g.o.f estimator distribution for the background toys in figure~\ref{fig:GOF}, with the red arrow corresponding to the location of the estimator for the data.
We find that the g.o.f estimator for the data has good compatibility with the background toys distribution.

% More on Cousins-Baker algorithm
...
The cross section pull is then defined as the difference between the measured cross section and the injected signal cross section divided by the measured cross section uncertainty.

% Resulting pull distributions
The median for the resulting cross section pulls for each signal model as a function of \MX is shown in figure~\ref{fig:biasTest_injectedSignal}.

...
The resonant background contributions from \WVt are added on top of the obtained \Wjets background in the signal region.
...
\begin{enumerate}
  ...
  This better constrains the background, which allows for improved sensitivity of the search.
  ...
  \item Simultaneous fitting of all \WV/\WH signals in the jet mass sideband, therefore allowing for easy implementation of exotic models such as Heavy Vector Triplets.
\end{enumerate}
...
\begin{itemize}
  ...
  The main contribution is $t\bar{t}$ events, along with SM diboson and single-$t$ production events.
...
% Signal overview
Because the MC samples for the various signals used in this analysis only sample several points for the resonance mass \MX, we employ interpolation methods to model the signal for any arbitrary resonance mass \MX within the range 0.8-$4.5\unit{TeV}$.
To do so, we derive each of the parameters for the signal shapes as functions of \MX, as well as the signal yield per pb of cross section as a function of \MX.

\subsubsection{Signal Shapes}

% Signal shapes
Each signal model takes the same functional form in the two-dimensional \MVV-\MJ space.
The signals are parameterized in 2D as the product of the two 1D \MVV and \MJ shapes for the jet mass and the resonance mass given by
\begin{equation}
...
Because the signal parameters do not vary significantly between production modes, we allow for substituting non-\VBF signal shapes with \VBF shapes, and vice versa:
\begin{itemize}
...
We also perform a closure test by converting the \MVV and \MJ projections of the $\MX=2\unit{TeV}$ version of the template of each signal model into 1D histograms, and compare them with the corresponding weighted MC distributions.
...
This is done in simulation by estimating the Gaussian mean and deviation of the resolution of $\MVV/\MVVpart$ in bins of the generated jet \pt, which can be seen in figure~\ref{fig:nonResScaleResMVV}.
...
We consider various metrics of behavior between categories, such as the correlation between \MVV and \MJ, as well as the average jet \pt as a function of the jet mass, which is shown in figure~\ref{fig:nonRes2DCorr} between the four combinations of HP/LP and nobb/bb categories.
The behavior is similar enough between the categories that they may be considered for merging.
For this analysis, we merge the e/mu and bb/nobb/vbf categories while keeping the HP/LP and HDy/LDy categories separate, for a total of four 2D conditional non-resonant templates.
...
Rather than using a kernel method as with the 2D conditional template, we instead use a coarsely binned histogram of weighted MC events for \MJ, and fit the result with a spline.
...
Figure~\ref{fig:1dtemplateVsReco_nonRes_MJ_Run2} shows the resulting 1D templates for the nobb category compared to the weighted MC distributions used to obtain them.
...
As in the non-resonant case, we find good agreement between the templates and the MC.

...
The Higgs mechanism is ultimately the result of the combining spontaneous symmetry breaking and local gauge invariance~\cite{Higgs:1964ia}, and this process accounts for the masses of the electroweak $W^\pm$ and $Z$ bosons, while also introducing the Higgs field itself.
...
The incompleteness of the Standard Model has driven theoretical developments of alternative models that resolve its shortcomings.
Some examples of such models are Grand Unified Theories, supersymmetric models, and warped extra dimensions~\cite{2010arXiv1005.1676L}.
...
Despite this, there are still other models based on more complex groups, such as $E_6$~\cite{Hewett1989193}.
...
There are many BSM theories that suggest that there are extra spatial dimensions.
Randall-Sundrum (RS) models are examples of this, which propose that there is an extra fifth spatial dimension that has a strongly warped geometry~\cite{PhysRevLett.83.4690}.
% Elaborate
...
One of the most natural is supersymmetry, as

\section{Search for a Heavy Diboson Resonance}
\label{sec:VBF}

This thesis present results on the search for a new heavy $X$ boson as part of efforts to resolve the hierarchy problem.
The work is largely based off of a previous search using the 2016 data collected by the LHC during Run 2~\cite{Sirunyan_2018}.
The previous analysis was conducted using only two benchmark signal models, and it was agnostic to the method of
...
\subsection{Vector Boson Fusion Production Method}

...
Rather, the Higgs is responsible for giving the massive gauge bosons their masses.
...
The issues caused by the hierarchy problem are more apparent when considering the mass corrections to the Higgs.
The Higgs mass $m_H$ is corrected through one-loop Feynman diagrams that are proportional to $m_H^2$~\cite{Martin_1998}, two examples of which can be seen in figure~\ref{fig:higgsFeyn}.
...
\begin{figure}[htbp]
  ...
  \caption{
    One-loop Feynman diagrams with a Higgs coupling to a fermion $f$ (left), and a scalar $S$ (right).
    Such diagrams contribute quadratic corrections $m_H^2$ to the Higgs mass.
  }
  \label{fig:higgsFeyn}
\end{figure}
...
The other vector boson decays hadronically through the process \Vtoqqbarpr and is reconstructed as a single massive jet, and there are two forward-facing jets resulting from vector boson fusion (\VBF) production processes.
...
% GBulk parameters
The Kaluza-Klein Radion and Bulk Graviton benchmark models make use of common parameters because they are both based off of RS theories.
...
\end{equation}
There are several parameters that describe the three variants of the HVT models, but for this analysis we are interested only in three: $g_V$, $c_H$, and $c_F$, with the latter two being the Higgs and fermion couplings, respectively.
...
The \VBF production channel was selected to further suppress background and identify signal events since the \VBF process uniquely results in forward-facing jets~\cite{rauch2016vectorboson}, and all benchmark models used in the analysis are capable of this production mechanism.
...
\begin{figure}[htbp]
  ...
  \caption{
    ...
    The search for an $X$ boson will involve looking for a final state in which there is a single merged jet from the \VorH boson, a lepton $\ell$ with its corresponding neutrino produced from the $W$ decay, and forward-facing jets for \VBF production processes.
  }
